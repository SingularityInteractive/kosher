    ...
    getFieldIds(fieldNames, function(result){
        if(!result){
            client({ 'method': 'POST', 'path': api+'search', entity:{
              "jql": "project = "+config.jiraProject+" AND issueType = '"+config.featuresIssueType+"'",
              "startAt": 0,
              "maxResults": 1000,
              "fields": [
                "summary",
                ...
                function(response) {
                    for(var i = 0; i < response.entity.issues.length; i++){
                        var featureIssue = response.entity.issues[i];

                        var feature = featureIssue.fields[fields[config.featuresField]]+"\r\n";
                        ...
                        client({ 'method': 'POST', 'path': api+'search', entity:{
                          ...
                          "fields": [
                            ...
                            function(response) {
                                for(var j = 0; j < response.entity.issues.length; j++){
                                    var scenarioIssue = response.entity.issues[j];

                                    console.log("  Creating scenario from: "+scenarioIssue.key);

                                    var scenario = scenarioIssue.fields[fields[config.scenarioField]];
