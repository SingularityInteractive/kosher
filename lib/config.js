...
var schema = {
    properties: {
        ...
        defaultSourceBranch: {
            description: 'Default remote branch to branch from',
    ...
    if(fs.existsSync(file)) {
        try{
            ...
            for(var prop in schema.properties){
                if(!config[prop] && !!schema.properties[prop].default)
                    config[prop] = schema.properties[prop].default;

                if(process.env[prop]) 
                    config[prop] = process.env[prop];

                if(prop = "featuresPath"){
                    var path = config[prop];
                    if(path.charAt(path.length-1) != '/') config[prop] += '/';
                }
            }
        } catch(e) {
