...
function generateFeature(featureIssue){
    if(!!featureIssue.fields[fields[config.featuresField]]){
        ...
        // Print feature
        var featureCopy = featureIssue.fields[fields[config.featuresField]];

        if(featureCopy.indexOf('Feature:') == -1){
            featureCopy = "  "+featureCopy.split("\r\n").join("\r\n  ");
            featureCopy = "Feature: "+featureIssue.fields.summary+"\r\n"+featureCopy;
        }
...
function generateScenario(scenarioIssue, createBranch) {
    if(!!scenarioIssue.fields[fields[config.scenarioField]]) {
        ...
        var scenarioCopy = scenarioIssue.fields[fields[config.scenarioField]];

        if(scenarioCopy.indexOf('Scenario:') == -1){
            scenarioCopy = "  "+scenarioCopy.split("\r\n").join("\r\n  ");
            scenarioCopy = "Scenario: "+scenarioIssue.fields.summary+"\r\n"+scenarioCopy;
        }
