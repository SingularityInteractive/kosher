                ...
                if(error){
                    ...
                    exec('git branch --no-track '+branchName+' '+config.defaultSourceBranch, function (error, stdout, stderr) {
                        ...
                        if(error){
                            ...
                            requests++;
                            exec('git push -u origin '+branchName, function (error, stdout, stderr) {
                                requests--;

                                if(error){
                                    console.log('branch could not be pushed');
                                    callback(1);
                                } else {
                                    console.log('branch pushed and upstream set to "origin/'+branchName+'"');
                                    callback(0);
                                }
                            });
                        }
